...
def load_data(data_dir):
    ...
    for sign in range(NUM_CATEGORIES):
        print('Current category:', sign)
        for img in os.listdir(os.path.join(data_dir, str(sign))):
            
            # Append labels
            labels.append(sign)
            
            # Load, resize, and append images
            image = cv2.imread(os.path.join(data_dir, str(sign), img))
...
def get_model():
    ...
    # Max pooling using 3x3 pool size
